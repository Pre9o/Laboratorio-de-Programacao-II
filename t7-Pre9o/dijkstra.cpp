...
struct Grafo {
  ...
  void dijkstra(const int fonte) {
    ...
    for(auto& v: grafo){
      v.second.chave = std::numeric_limits<float>::max();
      v.second.anterior = -1;
    }
    ...
    for(auto& v: grafo){
      heap.insere(&v.second);
    }
    heap.atualiza();

    while(heap.vazio() == false){
      auto u = heap.topo();
      heap.remove();

      for(auto& a: u->arestas){
...
TEST_CASE("mediumEWD.txt 10 <-> 247") {
    ...
    g.dijkstra(10);
    std::list<int> res;
    std::list<int> resp { 10, 106, 179, 79, 51, 18, 35, 12, 121, 158, 249, 189, 150, 190, 247 };
